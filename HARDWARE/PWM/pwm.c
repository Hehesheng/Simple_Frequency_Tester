...
//	GPIO_InitStructure_1.GPIO_Mode = GPIO_Mode_AF;        //复用功能
//	GPIO_InitStructure_1.GPIO_Speed = GPIO_Speed_100MHz;	//速度100MHz
//	GPIO_InitStructure_1.GPIO_OType = GPIO_OType_PP;      //推挽复用输出
//	GPIO_InitStructure_1.GPIO_PuPd = GPIO_PuPd_UP;        //上拉
//	GPIO_Init(GPIOF,&GPIO_InitStructure_1);              //初始化PF9

//		
//	TIM_TimeBaseStructure_1.TIM_Prescaler=psc_1;  //定时器分频
//	TIM_TimeBaseStructure_1.TIM_CounterMode=TIM_CounterMode_Up; //向上计数模式
//	TIM_TimeBaseStructure_1.TIM_Period=arr_1;   //自动重装载值
//	TIM_TimeBaseStructure_1.TIM_ClockDivision=TIM_CKD_DIV1; 
//	
//	TIM_TimeBaseInit(TIM14,&TIM_TimeBaseStructure_1);//初始化定时器14
//	
//	//初始化TIM14 Channel1 PWM模式	 
//	TIM_OCInitStructure_1.TIM_OCMode = TIM_OCMode_PWM1; //选择定时器模式:TIM脉冲宽度调制模式2
// 	TIM_OCInitStructure_1.TIM_OutputState = TIM_OutputState_Enable; //比较输出使能
//	TIM_OCInitStructure_1.TIM_OCPolarity = TIM_OCPolarity_Low; //输出极性:TIM输出比较极性低
//	TIM_OC1Init(TIM14, &TIM_OCInitStructure_1);  //根据T指定的参数初始化外设TIM1 4OC1
...
////	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOF, ENABLE); 	//使能PORTF时钟	
//	
//	GPIO_PinAFConfig(GPIOF,GPIO_PinSource8,GPIO_AF_TIM14); //GPIOF9复用为定时器14
//	
